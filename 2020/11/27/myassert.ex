defmodule Mac.AssertionError do
  @moduledoc """
  ...
  def message(exception) do
    "\n\n" <> Mac.Formatter.format_assertion_error(exception)
  end
end

defmodule Mac.MultiError do
  @moduledoc """
...
defmodule Mac.Assertions do
  ...
  defmacro my_assert({:match?, meta, [left, right]} = assertion) do
    code = escape_quoted(:assert, meta, assertion)
    ...
    quote do
      ...
      Mac.Assertions.assert(unquote(match?),
        right: right,
        left: left,
        expr: unquote(code),
        message: "match (match?) failed" <> Mac.Assertions.__pins__(unquote(pins)),
      ...
      refute unquote(match?),
        ...
        message:
          "match (match?) succeeded, but should have failed" <>
            Mac.Assertions.__pins__(unquote(pins)),
        context: {:match, unquote(pins)}
    end
  end
  ...
  defp translate_operator(kind, {op, meta, [left, right]} = expr, call, message, true, _caller) do
    ...
    quote do
      ...
      if Mac.Assertions.__equal__?(left, right) do
        Mac.Assertions.my_assert(false,
          left: left,
          expr: unquote(expr),
          message: unquote(message <> ", both sides are exactly equal")
        )
      else
        Mac.Assertions.my_assert(unquote(call),
          left: left,
          right: right,
          expr: unquote(expr),
  ...
  defp extract_args({root, meta, [_ | _] = args} = expr, env) do
    ...
    case Macro.expand_once(expr, env) do
      ...
      other ->
        {Mac.AssertionError.no_value(), other}
    end
  end

  defp extract_args(expr, _env) do
    {Mac.AssertionError.no_value(), expr}
  end
          ...
          case right do
            ...
            _ ->
              ...
              raise Mac.AssertionError,
                ...
                right: right,
                expr: expr,
                message: "match (=) failed" <> Mac.Assertions.__pins__(unquote(pins)),
                context: {:match, unquote(pins)}
          end
        end
      )
  ...
  defp assert_receive(pattern, timeout, failure_message, caller, code) do
    ...
    quote do
      timeout = Mac.Assertions.__timeout__(unquote(timeout), :assert_receive_timeout)
      ...
      raise Mac.AssertionError,
        ...
        message:
          "Assertion failed, no matching message after #{timeout}ms" <>
            Mac.Assertions.__pins__(pins) <> message,
      ...
      error ->
        ...
        cond do
          ...
          true ->
            ...
            reraise Mac.AssertionError, [message: message], __STACKTRACE__
        end
    else
  ...
  defp do_refute_receive(pattern, timeout, failure_message) do
    ...
    quote do
      timeout = Mac.Assertions.__timeout__(unquote(timeout), :refute_receive_timeout)
  ...
  def flunk(message \\ "Flunked!") when is_binary(message) do
    my_assert false, message: message
  end
end
